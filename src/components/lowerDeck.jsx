...
function LowerDeck({seats}) {
  ...
  return (
   <Box w={['100%','100%','45%']}>
    <Text textAlign={'center'} color={'gray.700'}>Lower Breath</Text>
       ...
       <Box display={rowOne.length>0 ? 'flex' : 'none'} justifyContent={'space-between'} flexDir={'row-reverse'} >
            {
                rowOne.map((el,ind)=>{
                    return <Box>
                        <Box boxShadow={'md'} p={1} bg={'blue.100'} display={el.breath=='LB' && 'none'} borderRadius={5}><MdAirlineSeatFlat color="tomato" size={25} /></Box>
